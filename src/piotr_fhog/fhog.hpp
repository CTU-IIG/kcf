...
//            cv::Mat desc(hb, wb, CV_32F, (H+hb*wb*i));
    ...
    static std::vector<cv::UMat> extract(const cv::UMat & img, int use_hog = 2, int bin_size = 4, int n_orients = 9, int soft_bin = -1, float clip = 0.2)
    {
        // d image dimension -> gray image d = 1
        // h, w -> height, width of image
        // full -> ??
...
//        }
        ...
        for (int x = 0; x < w; ++x) {
            for (int y = 0; y < h; ++y) {
                I[x*h + y] = img.getMat(cv::ACCESS_RW).at<float>(y, x)/255.f;
            }
        }
        ...
        //convert, assuming row-by-row-by-channel storage
        std::vector<cv::UMat> res;
        int n_res_channels = (use_hog == 2) ? n_chns-1 : n_chns;    //last channel all zeros for fhog
        res.reserve(n_res_channels);
        for (int i = 0; i < n_res_channels; ++i) {
            //output rows-by-rows
//            cv::UMat desc(hb, wb, CV_32F, (H+hb*wb*i));

            //output cols-by-cols
            cv::UMat desc(hb, wb, CV_32F);
            for (int x = 0; x < wb; ++x) {
                for (int y = 0; y < hb; ++y) {
                    desc.getMat(cv::ACCESS_RW).at<float>(y,x) = H[i*hb*wb + x*hb + y];
                }
            }
